...
namespace UI
{
    ...
    void printBox(const std::string &title, const std::string &content)
    {
        ...
        else
        {
            ...
            if (leftPad < 2)
                leftPad = 2; // Đảm bảo luôn có ít nhất 2 dấu gạch ở bên trái
    ...
    {
        std::string title = "CHAMPION!";
        std::string content = "Trainer: " + player.name + "\n";
        content += "Starter: " + player.starterName + "\n";
        content += "Badges: " + std::to_string(player.badges) + "/8\n";
        content += "Congratulations on becoming the Champion!";
